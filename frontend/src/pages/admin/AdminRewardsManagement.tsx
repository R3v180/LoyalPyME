...
// Version: 1.1.1 (Remove unused ActionLoading type import)
...
// Importar hook y tipo Reward (ActionLoading ya no se importa aquí)
import { useAdminRewards, Reward } from '../../hooks/useAdminRewards';
// Importar Formulario
import RewardForm from '../../components/AddRewardForm';
...
const AdminRewardsManagement: React.FC = () => {
    ...
    const {
        ...
        loading,
        error,
        actionLoading, // <-- Se usa el valor del estado, no el tipo importado
        fetchRewards,
        handleToggleActive,
        handleDeleteReward
    } = useAdminRewards();
    ...
    const confirmAndDeleteReward = useCallback((rewardId: string, rewardName: string) => {
        if (window.confirm(`¿Estás seguro de que quieres eliminar la recompensa "${rewardName}"? Esta acción no se puede deshacer.`)) {
        ...
        const rows = rewards.map((reward) => {
            ...
            return (
                <Table.Tr key={reward.id}>
                    ...
                    <Table.Td><Badge color={reward.isActive ? 'green' : 'gray'} variant="light" radius="lg" fz={{ base: 'xs', sm: 'sm' }}>{reward.isActive ? 'Activa' : 'Inactiva'}</Badge></Table.Td>
            ...
            <Table striped highlightOnHover withTableBorder withColumnBorders verticalSpacing="sm">
                <Table.Thead>
                    <Table.Tr>
                        ...
                        <Table.Th fz={{ base: 'xs', sm: 'sm' }}>Coste</Table.Th>
    ...
    const renderRewardsSectionContent = () => {
         ...
         if (!loading && rewards.length === 0 && !showAddForm) return <Text c="dimmed" ta="center" p="lg">Aún no has creado ninguna recompensa.</Text>;
