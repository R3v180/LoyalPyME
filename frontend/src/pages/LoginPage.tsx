...
// Version: 1.1.7 (Activate Forgot Password link)

import { useState, FormEvent } from 'react';
import { useNavigate, Link } from 'react-router-dom'; // Link ya estaba importado
...
function LoginPage() {
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setLoading(true); setError(null); const loginUrl = 'http://localhost:3000/auth/login'; console.log('>>> Attempting POST to:', loginUrl); try { const response = await axios.post<{ user: UserResponse; token: string }>( loginUrl, { email, password } ); const { user, token } = response.data; if (user && token) { localStorage.setItem('token', token); localStorage.setItem('user', JSON.stringify(user)); if (user.role === 'BUSINESS_ADMIN') { navigate('/admin/dashboard'); } else if (user.role === 'CUSTOMER_FINAL') { navigate('/customer/dashboard'); } else { setError('Rol de usuario desconocido recibido.'); } } else { setError('Respuesta inesperada del servidor.'); } } catch (err: any) { console.error('Login error:', err); if (err.response?.status === 401) { setError('Credenciales inválidas.'); } else if (err.response?.data?.message) { setError(err.response.data.message); } else if (err.request) { setError('No se pudo conectar con el servidor.'); } else { setError('Ocurrió un error durante el inicio de sesión.'); } } finally { setLoading(false); } };
                ...
                <form onSubmit={handleSubmit}>
                    <Stack gap="lg">
                        ...
                        <Group justify="space-between" mt="xl">
                             {/* --- CAMBIO: Convertir Anchor en Link --- */}
