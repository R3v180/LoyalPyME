...
const AdminRewardsManagement: React.FC = () => {
    ...
    const renderRewardsTable = () => {
        const rows = rewards.map((reward) => {
            ...
            // El <></> que devolvía actionIcons podría causar problemas con Tooltip.
            ...
            return (
                ...
                <Table.Tr key={reward.id}>
                    ...
                    <Table.Td>
                        {isMobile ? (
                            <Stack gap={2} align="flex-end">{/* No dejar espacio extra aquí */}
                                {editIcon}
                                {toggleIcon}
                                {deleteIcon}
                            </Stack>
                        ) : (
                            <Group gap="xs" justify="flex-end" wrap="nowrap">{/* No dejar espacio extra aquí */}
                                {editIcon}
                                {toggleIcon}
                                {deleteIcon}
                            </Group>
                ...
                </Table.Tr> // No dejar espacio extra aquí
            );
        });
