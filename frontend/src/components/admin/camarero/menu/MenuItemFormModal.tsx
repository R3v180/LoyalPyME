...
import {
    ...
    Text as MantineText, // Usaremos este alias consistentemente
        ...
        <>
                ...
                <form onSubmit={form.onSubmit(handleSubmitForm)}>
                    <Stack gap="md">
                        ...
                        <Stack gap="xs" mt="sm">
                            ...
                            {!showCropper && ( 
                                <AspectRatio ratio={ASPECT_RATIO_ITEM} maw={200}><Center bg="gray.1" style={{borderRadius:'var(--mantine-radius-md)'}}>{displayImageUrl ? 
                                    <MantineImage src={displayImageUrl} alt="Vista previa del ítem" radius="md" fallbackSrc="/placeholder-item.png"/> 
                            ...
                            {showCropper && ( 
                                <Box mt="md"> 
                                    ...
                                    ><img ref={imgRef} src={imgSrcForCropper} style={{maxHeight:'300px',display:imgSrcForCropper?'block':'none'}} onLoad={onImageLoadForCropper} alt="Recortar"/></ReactCrop> 
                                    <Button 
                        ...
                        <MultiSelect
                            ...
                            creatable 
                            getCreateLabel={(query: string) => `+ Añadir "${query}"`}
                            disabled={isSubmitting || isUploadingImage}
                        ...
                        {initialData && (
                            <Box mt="lg" pt="lg" style={{ borderTop: `1px solid ${theme.colors.gray[3]}` }}>
                                <Group justify="space-between" mb="sm">
                                    <Title order={5}>{t('adminCamarero.manageMenu.modifierGroupsTitle', 'Grupos de Modificadores')}</Title>
                                    <Button
                                        variant="outline"
                                        size="xs"
                                        ...
                                        disabled={isSubmitting || isUploadingImage}
                                    >
                                        {t('adminCamarero.manageMenu.manageGroupsButton', 'Gestionar Grupos')}
                                    </Button>
                                </Group>
                                <MantineText size="xs" c="dimmed" mb="md">
                                    {t('adminCamarero.manageMenu.modifierGroupsDescription', 'Define grupos de opciones para personalizar este ítem (ej: Tamaño, Extras, Salsas).')}
                                </MantineText>
                            </Box>
                        )}
            ...
            {initialData && menuItemIdForGroupsModal && (
                <ModifierGroupsManagementModal
                    ...
                    menuItemName={initialData.name_es || initialData.name_en || 'Ítem'}
                />
            )}
        </>
    );
