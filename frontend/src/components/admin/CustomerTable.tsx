...
// Version: 1.2.3 (Fix encoding, remove meta-comments)
...
// (Lógica interna de Th sin cambios)
...
interface CustomerTableProps {
    ...
    selectedRows: string[];
    onSort: (column: SortColumn) => void; // Usa el tipo importado
    onToggleFavorite: (customerId: string, currentIsFavorite: boolean) => void; // Cambiado a void si no devuelve promesa
    ...
    onToggleActive: (customer: Customer) => void; // Cambiado a void si no devuelve promesa
...
const CustomerTable: React.FC<CustomerTableProps> = ({
    ...
    // Mapeo de filas (Codificación corregida)
    const rows = customers.map((customer) => {
        ...
        return (
            <Table.Tr key={customer.id} bg={isSelected ? theme.colors.blue[0] : undefined}>
                ...
                <Table.Td>{customer.currentTier?.name || 'Básico'}</Table.Td> {/* Corregido: Básico */}
                <Table.Td>{new Date(customer.createdAt).toLocaleDateString()}</Table.Td>
                <Table.Td> <Badge color={isActive ? 'green' : 'red'} variant="light"> {isActive ? 'Activo' : 'Inactivo'} </Badge> </Table.Td> {/* Corregido: Activo */}
                <Table.Td>
            ...
            <Table striped highlightOnHover withTableBorder verticalSpacing="sm" className={classes.table}>
                <Table.Thead className={classes.thead}>
                    <Table.Tr>
                        ...
                        <Table.Th style={{ textAlign: 'right' }}>Acciones</Table.Th>{/* Corregido: Acción */}
                    </Table.Tr>
                </Table.Thead>
                <Table.Tbody>{rows.length > 0 ? rows : ( <Table.Tr><Table.Td colSpan={9}><Text c="dimmed" ta="center">No se encontraron clientes.</Text></Table.Td></Table.Tr> )}</Table.Tbody>
            </Table>
...
// End of File: frontend/src/components/admin/CustomerTable.tsx