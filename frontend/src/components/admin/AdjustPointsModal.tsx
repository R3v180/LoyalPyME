// filename: frontend/src/components/admin/customers/AdjustPointsModal.tsx
// Version: 1.0.2 (Fix API POST URL - remove duplicate /api)
...
import axiosInstance from '../../services/axiosInstance'; // Usando la ruta correcta que indicaste

// Interfaz CustomerData (sin cambios)
...
const AdjustPointsModal: React.FC<AdjustPointsModalProps> = ({
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        if (pointsToAdd === '' || pointsToAdd === 0) {
            setError('Debes indicar una cantidad de puntos a añadir o restar (no puede ser cero).');
            return;
        }
        if (!customer) {
            setError('No se ha seleccionado un cliente válido.');
            return;
        }
        ...
        try {
            ...
            notifications.show({
                title: 'Éxito',
                message: `Se han ajustado ${amount} puntos para ${customer.name || customer.email}.`,
                color: 'green',
    ...
    return (
        <Modal
            opened={opened}
            onClose={onClose}
            title={`Ajustar Puntos para ${customer.name || customer.email}`}
            centered
            radius="lg"
        ...
        >
            ...
            <form onSubmit={handleSubmit}>
                <Stack gap="md">
                    ...
                    <NumberInput
                        label="Puntos a Añadir/Restar"
                        placeholder="Ej: 50 o -20"
                        value={pointsToAdd}
                        onChange={(value) => setPointsToAdd(typeof value === 'number' ? value : '')}
                        allowNegative
                        required
                        radius="lg"
                        data-autofocus
                    ...
                    <Textarea
                        ...
                        placeholder="Ej: Compensación por retraso, Bonus especial..."
                        value={reason}
                    ...
                    />
                    {error && (
                        <Alert title="Error" color="red" icon={<IconAlertCircle size={16}/>} radius="lg" withCloseButton onClose={() => setError(null)}>
                            {error}
                        </Alert>
                    )}
                    <Group justify="flex-end" mt="md">
                        <Button variant="light" onClick={onClose} disabled={isSubmitting} radius="lg">
                            Cancelar
                        </Button>
