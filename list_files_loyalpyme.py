...
output_file = os.path.join(base_dir, "project-loyalpyme-module.txt")

# Carpetas ESPECÍFICAS a incluir (deben existir)
# Incluimos los módulos específicos y todo lo que sea 'shared'
include_only_dirs = {
    os.path.join(base_dir, "backend", "src", "modules", "loyalpyme"),
    os.path.join(base_dir, "frontend", "src", "modules", "loyalpyme"),
...
always_include_files = {
    ...
    os.path.join("backend", "src", "index.ts"),
    os.path.join("backend", "src", "routes", "index.ts"),
    os.path.join("frontend", "src", "routes", "index.tsx"),
    os.path.join("frontend", "src", "main.tsx"),
}
...
def main():
    print(f"Iniciando escaneo para el MÓDULO LOYALPYME en: {base_dir}")
    all_files = []
    ...
    try:
        with open(output_file, "w", encoding="utf-8") as outfile:
            outfile.write("# ÍNDICE DE ARCHIVOS (MÓDULO LOYALPYME + SHARED)\n\n")
