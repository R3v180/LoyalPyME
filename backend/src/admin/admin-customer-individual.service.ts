...
// Version: 1.1.0 (Add ActivityLog creation on points adjustment)
...
// --- adjustPointsForCustomer MODIFICADO ---
/**
 * Ajusta los puntos de un cliente específico y crea un log de actividad.
 */
export const adjustPointsForCustomer = async (
    ...
    amount: number,
    reason?: string | null
): Promise<User> => { // Devuelve User completo (puede ser útil para el controlador)
    ...
    try {
        // Usamos transacción para asegurar consistencia entre actualización y log
        const updatedUser = await prisma.$transaction(async (tx) => {
            // 1. Verificar cliente y pertenencia
            const customer = await tx.user.findUnique({
                where: { id: customerId },
                select: { businessId: true, id: true, email: true } // Seleccionar solo lo necesario
            });
            ...
            const userAfterUpdate = await tx.user.update({
                ...
                // No necesitamos seleccionar nada específico aquí si devolvemos el resultado final
            });
            console.log(`[ADM_CUST_IND_SVC - TX] Points adjusted successfully for customer ${userAfterUpdate.email} (${customerId}). New balance potentially: ${userAfterUpdate.points}`);

            // --- 3. NUEVO: Crear ActivityLog ---
            await tx.activityLog.create({
                data: {
                    userId: customerId,
                    businessId: adminBusinessId,
                    type: ActivityType.POINTS_ADJUSTED_ADMIN,
                    pointsChanged: amount, // El 'amount' que viene del controlador
                    description: reason || (amount > 0 ? 'Ajuste manual (suma)' : 'Ajuste manual (resta)') // Descripción por defecto si no hay razón
                    // No hay IDs relacionados en este caso
                }
            });
            console.log(`[ADM_CUST_IND_SVC - TX] ActivityLog created for admin points adjustment for user ${customerId}.`);
            // --- FIN NUEVO ---

            return userAfterUpdate; // Devolver el usuario actualizado al final de la tx
        });

        return updatedUser; // Devolver el resultado de la transacción

    } catch (error) {
        console.error(`[ADM_CUST_IND_SVC] Error adjusting points for customer ${customerId}:`, error);
...
export const assignRewardToCustomer = async ( customerId: string, adminBusinessId: string, rewardId: string, adminUserId: string ): Promise<GrantedReward> => {
     ...
     try { const grantedReward = await prisma.$transaction(async (tx) => { const customer = await tx.user.findUnique({ where: { id: customerId, businessId: adminBusinessId }, select: { id: true, email: true } }); if (!customer) { throw new Error(`Cliente con ID ${customerId} no encontrado o no pertenece a tu negocio.`); } const reward = await tx.reward.findUnique({ where: { id: rewardId, businessId: adminBusinessId }, select: { id: true, isActive: true, name: true } }); if (!reward) { throw new Error(`Recompensa con ID ${rewardId} no encontrada o no pertenece a tu negocio.`); } if (!reward.isActive) { throw new Error(`La recompensa "${reward.name}" (${rewardId}) no está activa y no puede ser asignada.`); } const newGrantedReward = await tx.grantedReward.create({ data: { userId: customerId, rewardId: rewardId, businessId: adminBusinessId, assignedById: adminUserId, status: 'PENDING', } }); console.log(`[ADM_CUST_IND_SVC] Reward ${reward.name} (${rewardId}) assigned successfully to customer ${customer.email} (${customerId}). GrantedReward ID: ${newGrantedReward.id}`); return newGrantedReward; }); return grantedReward; } catch (error) { console.error(`[ADM_CUST_IND_SVC] Error assigning reward ${rewardId} to customer ${customerId}:`, error); if (error instanceof Prisma.PrismaClientKnownRequestError) { throw new Error(`Error de base de datos al asignar recompensa: ${error.message}`); } if (error instanceof Error) { throw error; } throw new Error('Error inesperado al asignar la recompensa.'); }
};
