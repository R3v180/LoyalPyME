...
// Version: 1.3.0 (Add handler for updating admin notes)

import { Request, Response, NextFunction } from 'express';
import { UserRole } from '@prisma/client'; // UserRole usado en getAdminCustomers
...
import {
    ...
    updateAdminNotesForCustomer // <-- Importar la nueva función del servicio
    // GetCustomersOptions
...
// Handler para obtener detalles (sin cambios)
...
export const updateCustomerNotesHandler = async (req: Request, res: Response, next: NextFunction) => {
    const { customerId } = req.params;
    const { notes } = req.body; // Esperamos { "notes": "contenido..." } o { "notes": null }
    ...
    // Validar que 'notes' venga en el body, aunque sea null
    if (notes === undefined) {
        return res.status(400).json({ message: "Falta el campo 'notes' en el cuerpo de la petición." });
    }
     // Validar que 'notes' sea string o null
     if (notes !== null && typeof notes !== 'string') {
         return res.status(400).json({ message: "El campo 'notes' debe ser un texto o nulo." });
     }
    ...
    try {
        ...
        // Enviar respuesta exitosa (solo mensaje, o los datos actualizados si se prefiere)
        res.status(200).json({
            message: `Notas actualizadas correctamente para ${updatedCustomer.email}.`,
            // customer: { id: updatedCustomer.id, adminNotes: updatedCustomer.adminNotes } // Opcional: devolver notas
        });
    } catch (error) {
        // Manejar errores (ej. cliente no encontrado lanzado por el servicio)
...
export const changeCustomerTierHandler = async (req: Request, res: Response, next: NextFunction) => {
    // ... (código existente sin cambios)
     ...
     if (!adminBusinessId) { return res.status(403).json({ message: "Información de administrador no disponible." }); }
     if (!customerId) { return res.status(400).json({ message: "Falta el ID del cliente." }); }
     // Corrección: El body debe contener 'tierId', no 'newTierId' según la ruta original PUT
     if (!tierId || (typeof tierId !== 'string' && tierId !== null)) { // Permitir null si se quiere quitar tier
         return res.status(400).json({ message: "Falta el ID del nivel ('tierId') o no es válido (debe ser texto o nulo)." });
     }
     try {
